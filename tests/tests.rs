...
fn test_rests() {
    let composition = {
        ...
        let chord = staff.add_chord();
        chord.borrow_mut().add_note(Pitch::new(PitchName::A, 2), Duration::new(DurationType::Eighth, 0), None);
        chord.borrow_mut().add_note(Pitch::new_rest(), Duration::new(DurationType::Eighth, 0), None);
        chord.borrow_mut().add_note(Pitch::new(PitchName::E, 4), Duration::new(DurationType::Eighth, 0), None).borrow_mut().add_modification(NoteModificationType::Dynamic { dynamic: Dynamic::new(DynamicMarking::Forte, 0) });
